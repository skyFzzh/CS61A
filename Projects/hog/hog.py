...
def boar_brawl(player_score, opponent_score):
    ...
    x = player_score%10
    y =0
    if opponent_score>9 : y= opponent_score//10%10
    score = 3*abs(x-y)
    if score==0: 
        return 1
    return score
...
def sus_points(score):
    ...
    if (num_factors(score)==4 or num_factors(score)==3): 
        score +=1
        while (not is_prime(score)): score+=1
        return score
    return score
    # END PROBLEM 4
...
def is_always_roll(strategy, goal=GOAL):
    ...
    for i in range(goal):
        for j in range(goal):
            if num!= strategy(i,j): return False
    return True
    # END PROBLEM 7
...
def make_averaged(original_function, samples_count=1000):
    ...
    """
    # BEGIN PROBLEM 8
    def make_averaged_fuction(*args):
        sum=0
        for i in range(samples_count):
            sum += original_function(*args)
        return sum/samples_count
    return make_averaged_fuction
    # END PROBLEM 8
...
def sus_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    after_score = sus_points(score+boar_brawl(score,opponent_score))
    if (after_score-score)>=threshold: return 0
    return num_rolls  # Remove this line once implemented.
    # END PROBLEM 11
