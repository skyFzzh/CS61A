...
def about(subject):
    ...
    def check(ps):
        lists = []
        ps = lower(remove_punctuation(ps))
        lists += split(ps)
        for word in lists:
...
def accuracy(typed, source):
    ...
    # BEGIN PROBLEM 3
    "*** YOUR CODE HERE ***"
    if len(typed_words)==0 and len(source_words)==0: return 100.0
    if len(typed_words)==0: return 0.0
    count = 0
    for i in range(len(source_words)):
        if i >= len(typed_words): break
        if source_words[i]==typed_words[i]: count+=1
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    for word in word_list:
        if typed_word == word: return typed_word
        nowmin = diff_function(typed_word,word,limit)
        if (nowmin == minv and ans =='') or nowmin < minv:
            ans = word
            minv = min(minv,nowmin)
    if ans == '': ans= typed_word
    return ans
...
def feline_fixes(typed, source, limit):
    ...
    """
    # BEGIN PROBLEM 6
    if typed == "": return len(source)
    if source == "": return len(typed) 
    if typed[0] != source[0]:
        limit = limit-1
...
def minimum_mewtations(typed, source, limit):
    ...
    if typed[0] == source[0]: # Feel free to remove or add additional cases
        ...
        remove_diff = minimum_mewtations(typed, source[1:], limit - 1)
        add_diff = minimum_mewtations(typed[1:], source, limit - 1) 
        substitute_diff = minimum_mewtations(typed[1:], source[1:], limit - 1) 
