...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 8b
            if self.is_container: # normal ant already exists
                if not place.ant.is_container:
                    self.store_ant(place.ant)
                    place.ant = self
                else:
                    assert place.ant is None, 'Two ants in {0}'.format(place)
            else: # container ant already exists
    ...
    def double(self):
        ...
        if self.is_container and self.ant_contained and not self.ant_contained.is_doubled:
            self.ant_contained.damage = self.ant_contained.damage * 2
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        if self.place.bees != [] and self.lower_bound == 0:
            return random_bee(self.place.bees)  
        else:
            now, index = self.place.entrance, 1
            while not now.is_hive:
                if now.bees != [] and self.lower_bound <= index <= self.upper_bound:
                    return random_bee(now.bees)
                else:
                    now = now.entrance
                    index += 1
            return None
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        # reflected damage
        bee_lst = list(self.place.bees) # use "list()" constructor
        for bee in bee_lst:
            bee.reduce_health(amount)
        Ant.reduce_health(self, amount)
        if self.place == None:
            bee_lst = list(original_place.bees) # use "list()" constructor
            for bee in bee_lst:
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        if self.place.bees != [] and self.turn_to_chew == 0:
            eaten_bee = random_bee(self.place.bees)
            eaten_bee.reduce_health(eaten_bee.health)
            self.turn_to_chew = self.chewing_turns
        if self.turn_to_chew >0:
            self.turn_to_chew -=1
